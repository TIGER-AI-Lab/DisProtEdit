...
def extract(dataloader):
    ...
    return protein_repr_array, structure_repr_array, functional_repr_array, protein_seq_list, text_S_seq_list, text_F_seq_list
...
if __name__ == "__main__":
    ...
    structure_repr = text2latent_S_model(description_repr)
    functional_repr = text2latent_F_model(description_repr)
    structure_repr = structure_repr.detach().cpu().numpy()
    functional_repr = functional_repr.detach().cpu().numpy()
    description_repr = np.concatenate([structure_repr, functional_repr], axis=1)
