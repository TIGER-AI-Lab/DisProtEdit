...
if __name__ == "__main__":
    ...
    if general_args.pretrained_model in ["ProteinDT"]:
        ...
        try:
            ...
            print("protein_model.pth not found, loading from pretrained folder... Note that you should not be training on this model.")
            try:
                model = model_mapping[general_args.task_name].from_pretrained(
                    general_args.pretrained_folder,
                    mean_output=general_args.mean_output,
                    num_labels=len(processor.get_labels()),
                )
            except Exception as e:
                print("Error: Failed to load model using both approaches.")
                print(f"First approach error: FileNotFoundError - protein_model.pth not found")
                print(f"Second approach error: {str(e)}")
                raise e
    ...
    if general_args.task_name == 'contact':
        ...
        print("Using OntoProteinTrainer for contact prediction")
        ...
        print("Using Trainer for other tasks")
        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=train_dataset,
    ...
    if training_args.do_train:
        ...
        print("Save model to {}".format(training_args.output_dir))
        tokenizer.save_pretrained(training_args.output_dir)
        print("Save tokenizer to {}".format(training_args.output_dir))

    # trainer.compute_metrics = metrics_mapping(args.task_name)
    log_file = os.path.join(training_args.output_dir, f"eval_results_{general_args.task_name}.txt")
    with open(log_file, "w") as f:
        if general_args.task_name == 'remote_homology':
            ...
            f.write(f"metrics_fold: {metrics_fold_family}\n")
            f.write(f"metrics_family: {metrics_family_family}\n")
            f.write(f"metrics_superfamily: {metrics_superfamily_family}\n")
            ...
            f.write(f"cb513: {metrics_cb513}\n")
            f.write(f"ts115: {metrics_ts115}\n") 
            f.write(f"casp12: {metrics_casp12}\n")
        else:
            predictions_family, input_ids_family, metrics_family = trainer.predict(test_dataset)
            print("metrics", metrics_family)
            f.write(f"metrics: {metrics_family}\n")